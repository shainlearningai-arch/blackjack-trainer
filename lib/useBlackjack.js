...
import { getBasicStrategyAction, ACTION_LABELS, explainAction, countContext, getInsuranceAdvice, explainInsuranceMistake } from './basicStrategy';
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'DEAL': {
      ...
      const lastNetProfit = result === 'lose' ? -state.bet
        : result === 'blackjack' ? Math.floor(state.bet * 1.5)
        : 0;
    ...
    case 'INSURANCE_ACTION': {
      ...
      if (state.playerHadBlackjack) {
        ...
        return {
          ...
          phase: 'over',
          result: 'blackjack',
          lastNetProfit: Math.floor(state.bet * 1.5),
          insuranceTook: take,
          insuranceResult,
          insuranceMistake,
        };
    ...
    case 'PLAYER_ACTION': {
      ...
      const mistakes = isCorrect ? [] : [{
        ...
        countNote: countContext(playerHand, dealerUpcard, tc),
      }];
      ...
      if (move === 'R') {
        return {
          ...
          lastNetProfit: Math.floor(state.bet / 2),
      ...
      if (move === 'D') {
        ...
        const handBets = state.handBets.map((b, i) => i === state.activeHandIndex ? b * 2 : b);
        const nextPhase = isBust(newHand) ? 'over' : 'dealer';
        return {
          ...state, shoe, runningCount: rc, playerHands: newHands, bankroll, handBets,
          phase: nextPhase,
          result: isBust(newHand) ? 'bust' : null,
          lastNetProfit: isBust(newHand) ? -handBets[state.activeHandIndex] : state.lastNetProfit,
      ...
      if (move === 'P') {
        ...
        // Can re-split if the newly dealt card matches the original (up to 6 hands total)
        const canSplit = newHands.length < 6
          && normRank(c1.rank) === normRank(newC1.rank)
          && bankroll >= state.bet;
        return {
          ...state, shoe, runningCount: rc, playerHands: newHands, bankroll, handBets,
          canDouble: bankroll >= state.bet, canSurrender: false, canSplit,
      ...
      if (move === 'S') {
        ...
        if (nextIndex < state.playerHands.length) {
          ...
          const canSplit = state.playerHands.length < 6
            && nextHand.length === 2
            && normRank(nextHand[0].rank) === normRank(nextHand[1].rank)
            && state.bankroll >= state.bet;
          return { ...state, activeHandIndex: nextIndex, canDouble: state.bankroll >= state.bet, canSurrender: false, canSplit, lastMistakes: mistakes, stats: newStats };
        }
      ...
      if (isBust(newHand) || handTotal(newHand) === 21) {
        const nextIndex = state.activeHandIndex + 1;
        if (nextIndex < state.playerHands.length) {
          const nextHand = newHands[nextIndex];
          const canSplit = newHands.length < 6
            && nextHand.length === 2
            && normRank(nextHand[0].rank) === normRank(nextHand[1].rank)
            && state.bankroll >= state.bet;
          const nextCanDouble = nextHand.length === 2 && state.bankroll >= state.bet;
          return { ...state, shoe, runningCount: rc, playerHands: newHands, activeHandIndex: nextIndex, canDouble: nextCanDouble, canSurrender: false, canSplit, lastMistakes: mistakes, lastCardCountHints: hints, stats: newStats };
        }
        // If every hand is busted, skip dealer play entirely — the house wins automatically
        if (newHands.every(h => isBust(h))) {
          const totalLost = state.handBets.reduce((s, b) => s + b, 0);
          return { ...state, shoe, runningCount: rc, playerHands: newHands, phase: 'over', result: 'bust', lastNetProfit: -totalLost, lastMistakes: mistakes, lastCardCountHints: hints, stats: newStats };
        }
    ...
    case 'DEALER_PLAY': {
      ...
      if (hand.some(c => c.faceDown)) {
        ...
        hand = hand.map(c => c.faceDown ? { ...c, faceDown: false, justRevealed: true } : c);
      ...
      let netProfit = 0;
      results.forEach((r, i) => {
        const handBet = state.handBets?.[i] ?? state.bet;
        if (r === 'win')  { bankroll += handBet * 2; netProfit += handBet; }
        else if (r === 'push') { bankroll += handBet; }
        else               { netProfit -= handBet; }
      });

      const overallResult = netProfit > 0 ? 'win' : netProfit < 0 ? 'lose' : 'push';

      return { ...state, shoe, runningCount: rc, dealerHand: hand, phase: 'over', result: overallResult, bankroll, lastNetProfit: netProfit };
    }
    ...
    case 'RESET':
      return { ...initialState(), stats: state.stats, bankroll: 1000, runningCount: state.runningCount, shoe: state.shoe, bet: state.bet, lastBet: state.bet };
